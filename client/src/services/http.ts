...
// In production all fetch() calls use relative /api/... paths — Vercel's
// rewrite proxies them to Render server-side, so the Set-Cookie lands on the
// Vercel domain (same-site as the SPA) and is sent reliably on every refresh.
// VITE_API_ORIGIN is still read for OAuth browser-navigation URLs, which must
// bypass the Vercel proxy (Vercel follows upstream redirects, breaking OAuth).
const OAUTH_ORIGIN = (import.meta.env.VITE_API_ORIGIN as string | undefined)?.replace(/\/+$/, '') ?? '';

/** Used only for full-page OAuth navigations that must bypass the Vercel proxy. */
export const apiOrigin = () => OAUTH_ORIGIN;
